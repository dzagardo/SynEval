# experiments/run_dp_evaluation_adult.sh
#!/bin/bash

###############################################################################
# Adult Differential Privacy Evaluation Runner - Bash Wrapper
# 
# Convenience wrapper for running the Adult Income specific DP evaluation
# pipeline with the accompanying configuration and Python entry point.
###############################################################################
...
# Default values
CONFIG_FILE="configs/cross_group_dp_adult.yaml"
PARALLEL=true
WORKERS=4
LOG_LEVEL="INFO"
PYTHON_CMD="python3"
...
Run Adult DP evaluation experiments with SynEval

OPTIONS:
    -c, --config FILE       Path to configuration file (default: configs/cross_group_dp_adult.yaml)
...
run_evaluation() {
    local cmd="$PYTHON_CMD $SCRIPT_DIR/run_dp_evaluation_adult.py"
    ...
    print_info "Starting Adult DP evaluation runner..."
...
# Main execution
echo "============================================================"
echo "    Adult DP Evaluation Runner"
echo "============================================================"
echo
...
if [ "$DRY_RUN" = true ]; then
    ...
    cmd="$PYTHON_CMD $SCRIPT_DIR/run_dp_evaluation_adult.py"
    cmd="$cmd --config $CONFIG_FILE"
    cmd="$cmd --log-level $LOG_LEVEL"
...
if [ $exit_code -eq 0 ]; then
    echo
    print_info "Results saved to: $PROJECT_ROOT/reports/dp_evaluation_adult/"
    print_info "View execution summary: $PROJECT_ROOT/reports/dp_evaluation_adult/execution_summary.json"
fi
